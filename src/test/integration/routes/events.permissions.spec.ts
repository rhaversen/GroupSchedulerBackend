...
describe('Event: transitions & membership', function () {
	...
	it('scheduling status default; scheduling with scheduledTime rejected', async function () {
		...
		// valid transition
		// already scheduling by default; patch should no-op
	...
	it('cannot create confirmed without scheduledTime; cannot change scheduledTime after confirmation', async function () {
		const { cookie } = await register('Bob', 'b@example.com')
		const tw = futureWindow()
		// attempt invalid direct confirmed creation
		const badCreate = await agent().post('/api/v1/events').set('Cookie', cookie).send({ name: 'E2', description: 'D', timeWindow: tw, duration: 3600000, status: 'confirmed' })
		expect(badCreate).to.have.status(400)
		// create scheduling then confirm with scheduledTime
		const createRes2 = await agent().post('/api/v1/events').set('Cookie', cookie).send({ name: 'E2b', description: 'D', timeWindow: tw, duration: 3600000 })
		expect(createRes2).to.have.status(201)
		const id = createRes2.body._id
		const confirmed = await agent().patch(`/api/v1/events/${id}`).set('Cookie', cookie).send({ status: 'confirmed', scheduledTime: tw.start + 5000 })
		expect(confirmed).to.have.status(200)
		// attempt to change scheduledTime after confirmation
		const change = await agent().patch(`/api/v1/events/${id}`).set('Cookie', cookie).send({ scheduledTime: tw.start + 8000 })
		expect(change).to.have.status(400)
	})

	it('confirmed -> cancel allowed; cancel is terminal for all further modifications', async function () {
		...
		const id = createRes.body._id
		const confirmRes = await agent().patch(`/api/v1/events/${id}`).set('Cookie', cookie).send({ status: 'confirmed', scheduledTime })
		...
		// further update attempt (any field)
	...
	it('admin cannot promote via role endpoint but can demote admin -> participant', async function () {
		...
		const tw = futureWindow()
		const createRes = await agent().post('/api/v1/events').set('Cookie', creatorCookie).send({ name: 'M4', description: 'D', timeWindow: tw, duration: 3600000, members: [{ userId: creatorId, role: 'creator' }, { userId: adminId, role: 'admin' }] })
		const eventId = createRes.body._id
		// Admin attempts to promote themselves to creator (should fail 400/403)
		const promoteAttempt = await agent().patch(`/api/v1/events/${eventId}/members/role`).set('Cookie', adminCookie).send({ userId: adminId, role: 'creator' })
		expect([400,403]).to.include(promoteAttempt.status)
		// Admin demotes self to participant (allowed)
		const demoteAttempt = await agent().patch(`/api/v1/events/${eventId}/members/role`).set('Cookie', adminCookie).send({ userId: adminId, role: 'participant' })
	...
	it('public event accessible to anonymous', async function () {
		...
		const tw = futureWindow()
		const createRes = await agent().post('/api/v1/events').set('Cookie', c1).send({ name: 'PubEvt', description: 'D', timeWindow: tw, duration: 3600000, visibility: 'public' })
		const id = createRes.body._id
